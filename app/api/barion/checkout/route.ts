  ...
  try {
    ...
    if (!(await isProfileComplete(user.id))) {
      return Response.json(
        { ok: false, error: "A feltöltéshez előbb töltsd ki a profil és cím adatokat a fiókodban." },
        { status: 400 },
      );
    }
    ...
    if (!deviceIdentifier) {
      return Response.json(
        { ok: false, error: "Válassz vagy adj meg készülék azonosítót." },
        { status: 400 },
      );
    }
    ...
    if (travelDestination.length < 2) {
      return Response.json(
        { ok: false, error: "Add meg az úticélt (ország / régió)." },
        { status: 400 },
      );
    }
    ...
    if (!ownedDevice) {
      return Response.json(
        { ok: false, error: "Ez a készülék nem tartozik a fiókodhoz, vagy nem létezik." },
        { status: 403 },
      );
    }
    ...
    const paymentRequestId = generatePaymentRequestId("topup");
    const discountNote =
    ...
    if (dbError) {
      console.error("[barion] Pending topup rekord mentése sikertelen:", dbError.message);
    }
