...
export function ImportDevicesForm() {
  ...
  return (
    <section className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
      <h3 className="text-lg font-semibold">Eszközök CSV import</h3>
      <p className="mt-2 text-sm text-muted">
        Azonosítók és kategóriák feltöltése a devices táblába.
      </p>
      ...
      <form onSubmit={onSubmit} className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          {isLoading ? "Import folyamatban…" : "Eszközök import indítása"}
        </button>
      </form>
        ...
        >
          {result.ok ? (
            <div className="space-y-1">
              ...
              <p>Új eszközök: {result.insertedRows}</p>
              <p>Kihagyott (létező): {result.skippedRows}</p>
            </div>
          ) : (
