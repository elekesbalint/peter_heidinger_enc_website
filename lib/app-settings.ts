...
const DEFAULTS: Record<string, string> = {
  ...
  fx_eur_to_huf: "400",
  fx_hrk_to_huf: "53",
  topup_package_1_huf: "15000",
  topup_package_2_huf: "25000",
...
export function getTopupPackagesFromSettings(map: Record<string, string>): number[] {
  const a = getIntSetting(map, "topup_package_1_huf", 15000);
  const b = getIntSetting(map, "topup_package_2_huf", 25000);
  const c = getIntSetting(map, "topup_package_3_huf", 45000);
